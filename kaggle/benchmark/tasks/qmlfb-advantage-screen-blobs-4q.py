...
Screen verdict on this data: quantum candidate (g = 266.49).
"""
...
# %%
REF = {'g': 266.49259025862347, 'category': 'quantum candidate', 'keep': True}
PARAMS = {'kind': 'blobs', 'n_features': 4}
...
@kbench.task(name="qmlfb-advantage-screen-blobs-4q", description='Run the geometric-difference screen and make the conservative deployment call.')
def qmlfb_advantage_screen_blobs_4q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=80, n_features=4, seed=8, class_sep=2.5)
...
# %%
qmlfb_advantage_screen_blobs_4q.run(kbench.llm)
