...
Screen verdict on this data: inconclusive (g = 611.60).
"""
...
# %%
REF = {'g': 611.6020759108915, 'category': 'inconclusive', 'keep': True}
PARAMS = {'kind': 'moons', 'n_features': 2}
...
@kbench.task(name="qmlfb-advantage-screen-moons-2q", description='Run the geometric-difference screen and make the conservative deployment call.')
def qmlfb_advantage_screen_moons_2q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("moons", n_samples=80, n_features=2, seed=3, class_sep=2.5)
...
# %%
qmlfb_advantage_screen_moons_2q.run(kbench.llm)
