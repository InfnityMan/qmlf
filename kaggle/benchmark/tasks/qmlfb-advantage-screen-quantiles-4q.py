# %%
"""Tier B -- Quantum-advantage screening judgment (Huang et al. 2021)

Screen verdict on this data: quantum candidate (g = 153.91).
"""
...
# %%
REF = {'g': 153.9120336680169, 'category': 'quantum candidate', 'keep': True}
PARAMS = {'kind': 'quantiles', 'n_features': 4}



PROMPT = """\
Before spending hardware time, a professional runs the geometric-difference screen of Huang et al. (Nature Commun. 2021). qmlf.quantum_advantage_report(X, y) returns a dict with 'geometric_difference', 'g_matchable_below', 'g_advantage_scale', 's_classical', 's_quantum' and 'verdict' (a string beginning with 'classical kernel can match', 'quantum candidate', or 'inconclusive'). Write
    def screen(X, y):
        return {"g": <float>, "verdict_category": "classical-matchable" | "quantum-candidate" | "inconclusive", "recommendation": "run-quantum" | "stay-classical"}
Policy (conservative, as the paper advises): recommend "run-quantum" ONLY for "quantum-candidate". Return only one ```python code block.\
"""


_CATEGORY = {"classical kernel can match": "classical-matchable", "quantum candidate": "quantum-candidate", "inconclusive": "inconclusive"}
def score(source, data, ref, params):
    import qmlf
    X_train, X_test, y_train, y_test = data
    X, y = X_train, y_train
    r = qmlf.quantum_advantage_report(X, y)
    cat = _CATEGORY[r["verdict"].split(":")[0]]; rec = "run-quantum" if cat == "quantum-candidate" else "stay-classical"
    result, error = _run_model_function(source, "screen", (X, y))
    g_ok = cat_ok = rec_ok = False
    if error is None:
        try:
            g = float(result["g"]); g_ok = abs(g - r["geometric_difference"]) <= 0.15 * r["geometric_difference"]
            cat_ok = result["verdict_category"] == cat; rec_ok = result["recommendation"] == rec
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (g_ok, f"g must be within 15% of the screen's value {r['geometric_difference']:.2f}"),
              (cat_ok, f"verdict category must be {cat!r}"),
              (rec_ok, "recommendation must follow the conservative policy (run-quantum only for quantum-candidate)")]
    return {"reference_g": r["geometric_difference"], "reference_category": cat, "error": error}, checks



@kbench.task(name="qmlfb-advantage-screen-quantiles-4q", description='Run the geometric-difference screen and make the conservative deployment call.')
def qmlfb_advantage_screen_quantiles_4q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("quantiles", n_samples=80, n_features=4, seed=64, class_sep=2.5)
...
# %%
qmlfb_advantage_screen_quantiles_4q.run(kbench.llm)
