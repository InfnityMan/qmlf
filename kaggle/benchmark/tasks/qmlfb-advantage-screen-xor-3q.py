...
Screen verdict on this data: quantum candidate (g = 278.33).
"""
...
# %%
REF = {'g': 278.3314230925884, 'category': 'quantum candidate', 'keep': True}
PARAMS = {'kind': 'xor', 'n_features': 3}
...
@kbench.task(name="qmlfb-advantage-screen-xor-3q", description='Run the geometric-difference screen and make the conservative deployment call.')
def qmlfb_advantage_screen_xor_3q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("xor", n_samples=80, n_features=3, seed=62, class_sep=2.5)
...
# %%
qmlfb_advantage_screen_xor_3q.run(kbench.llm)
