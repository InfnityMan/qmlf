# %%
"""Tier A -- Per-feature bandwidth against hidden noise features

4 informative + 8 noise features. Best scalar 0.607; ARD 0.750.
"""
...
# %%
REF = {'scalar_best': 0.6071428571428571, 'ard_best': 0.75, 'ard_by_inf_bw': {'0.25': 0.6428571428571429, '0.1': 0.5357142857142857, '0.05': 0.75}, 'acc_bar': 0.68, 'keep': True}
PARAMS = {'n_informative': 4, 'n_features': 12}



PROMPT = """\
Binary classification, 12 continuous features. Some carry signal, some are pure noise; you are not told which. A fidelity kernel encodes every feature as an angle, so noise features destroy same-class similarity. qmlf.QuantumKernel accepts a per-feature bandwidth vector: QuantumKernel(bandwidth=np.array([... 12 entries ...])) scales each feature's angle independently (applied after any whitening). A fitted kernel gives compute_kernel_matrix(X) and compute_kernel_matrix(X_test, X_train) for sklearn SVC(kernel="precomputed"). Write
    def solve(X_train, y_train, X_test):
        return {"predictions": <test predictions>, "bandwidth_vector": <the per-feature bandwidth you used>}
Scoring: accuracy >= 0.68 AND the bandwidth on the noise features must be <= 0.30x the mean bandwidth of the informative ones. Identify noise features from the training data (univariate F-scores, mutual information, ...). The best scalar bandwidth scores well below the bar; only per-feature scaling reaches it. Return only one ```python code block.\
"""
...
def score(source, data, ref, params):
    ...
    n_inf, n_tot = params["n_informative"], params["n_features"]
    ...
    if error is None:
        try:
            ...
            if bw.shape != (n_tot,): raise ValueError(f"bandwidth_vector must have {n_tot} entries, got {bw.shape}")
            ratio = float(bw[n_inf:].mean() / bw[:n_inf].mean())
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (ratio <= 0.30, f"Noise-feature bandwidth must be <= 0.30x informative mean. Got ratio {ratio:.3f}"),
              (acc >= ref["acc_bar"], f"Accuracy >= {ref['acc_bar']} (best scalar {ref['scalar_best']:.3f}, ARD {ref['ard_best']:.3f}). Got {acc:.3f}")]
    return {"accuracy": acc, "noise_to_informative_ratio": ratio, "error": error}, checks



@kbench.task(name="qmlfb-ard-4-informative-of-12", description='Discover hidden noise features and suppress them with a per-feature quantum-kernel bandwidth.')
def qmlfb_ard_4_informative_of_12(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("shift", n_samples=110, n_features=12, n_informative=4, seed=38)
...
# %%
qmlfb_ard_4_informative_of_12.run(kbench.llm)
