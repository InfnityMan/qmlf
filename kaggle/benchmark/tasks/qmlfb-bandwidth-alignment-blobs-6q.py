...
Principle alignment -> 0.05 on this training set.
"""
...
# %%
REF = {'expected': 0.05, 'keep': True}
PARAMS = {'principle': 'alignment', 'kind': 'blobs', 'n_features': 6}
...
@kbench.task(name="qmlfb-bandwidth-alignment-blobs-6q", description='Select the fidelity-kernel bandwidth by the requested principle and report exactly what it yields.')
def qmlfb_bandwidth_alignment_blobs_6q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=90, n_features=6, seed=51)
...
# %%
qmlfb_bandwidth_alignment_blobs_6q.run(kbench.llm)
