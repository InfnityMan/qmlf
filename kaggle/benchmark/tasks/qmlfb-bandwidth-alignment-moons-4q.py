# %%
"""Tier A -- Bandwidth selection by a named principle

Principle alignment -> 0.02 on this training set.
"""
...
# %%
REF = {'expected': 0.02, 'keep': True}
PARAMS = {'principle': 'alignment', 'kind': 'moons', 'n_features': 4}



PROMPT = """\
A reviewer asks for the fidelity-kernel bandwidth selected by a specific principle on this training set: kernel-target alignment: qmlf.QuantumKernel(bandwidth='alignment').fit(X_train, y_train) sets .bandwidth_ to the grid value maximising centered alignment with the labels. `qmlf` is installed. Write
    def select_bandwidth(X_train, y_train):
        return <the float bandwidth that principle yields>
The value is checked exactly (to 1e-9) against the principle run on the same data, so use the library's implementation of it rather than re-deriving an approximation. Return only one ```python code block.\
"""
...
def score(source, data, ref, params):
    ...
    result, error = _run_model_function(source, "select_bandwidth", (X_train, y_train))
    value, exact = None, False
    if error is None:
        try:
            value = float(result); exact = abs(value - ref["expected"]) <= 1e-9 * max(1.0, abs(ref["expected"]))
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (exact, f"The {params['principle']} principle on this training set gives exactly {ref['expected']:.10g}; got {value}")]
    return {"bandwidth": value, "expected": ref["expected"], "error": error}, checks



@kbench.task(name="qmlfb-bandwidth-alignment-moons-4q", description='Select the fidelity-kernel bandwidth by the requested principle and report exactly what it yields.')
def qmlfb_bandwidth_alignment_moons_4q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("moons", n_samples=90, n_features=4, seed=52)
...
# %%
qmlfb_bandwidth_alignment_moons_4q.run(kbench.llm)
