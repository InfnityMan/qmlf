...
Principle cv -> 0.02 on this training set.
"""
...
PARAMS = {'principle': 'cv', 'kind': 'blobs', 'n_features': 6}



PROMPT = """\
A reviewer asks for the fidelity-kernel bandwidth selected by a specific principle on this training set: the deterministic stratified cross-validation sweep that qmlf.QuantumClassifier(bandwidth='auto') runs at fit time (its chosen value is exposed as .bandwidth_). `qmlf` is installed. Write
...
@kbench.task(name="qmlfb-bandwidth-cv-blobs-6q", description='Select the fidelity-kernel bandwidth by the requested principle and report exactly what it yields.')
def qmlfb_bandwidth_cv_blobs_6q(llm) -> float:
    _ensure_qmlf()
...
# %%
qmlfb_bandwidth_cv_blobs_6q.run(kbench.llm)
