...
Principle median -> 0.282678 on this training set.
"""
...
# %%
REF = {'expected': 0.28267809990916337, 'keep': True}
PARAMS = {'principle': 'median', 'kind': 'shift', 'n_features': 5}



PROMPT = """\
A reviewer asks for the fidelity-kernel bandwidth selected by a specific principle on this training set: the median heuristic: qmlf.QuantumKernel(bandwidth='median').fit(X_train) sets .bandwidth_ to 1 / median pairwise distance of the prepared training angles. `qmlf` is installed. Write
...
@kbench.task(name="qmlfb-bandwidth-median-shift-5q", description='Select the fidelity-kernel bandwidth by the requested principle and report exactly what it yields.')
def qmlfb_bandwidth_median_shift_5q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("shift", n_samples=90, n_features=5, seed=53)
...
# %%
qmlfb_bandwidth_median_shift_5q.run(kbench.llm)
