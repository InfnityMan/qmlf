...
3-atom geometry with nuclear charges.
"""
...
@kbench.task(name="qmlfb-chemistry-3atoms", description="Produce qmlf's charge-aware Coulomb descriptor and model Hamiltonian for a geometry.")
def qmlfb_chemistry_3atoms(llm) -> float:
    _ensure_qmlf()
    data = make_geometry(seed=10, n_atoms=3)
...
# %%
qmlfb_chemistry_3atoms.run(kbench.llm)
