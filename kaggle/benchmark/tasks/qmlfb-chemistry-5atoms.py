# %%
"""Tier E -- Molecular descriptor and model Hamiltonian

5-atom geometry with nuclear charges.
"""
...
PROMPT = """\
For a 5-atom geometry (coordinates array (n, 3)) with nuclear charges (length n), produce qmlf's charge-aware Coulomb-matrix descriptor (sorted eigenvalue spectrum) and its geometry-derived model Hamiltonian matrix. qmlf.create_advanced_chem_layer(num_atoms=n) has build_coulomb_matrix(coordinates, charges), molecular_descriptor(coordinates, charges) and build_hamiltonian(coordinates, charges); charges are optional in the API and default to ones -- forgetting to pass them is the classic mistake. Write
    def describe(coordinates, charges):
        return {"descriptor": <length-n array>, "hamiltonian": <(n, n) array>}
Checked to 1e-9 against the library on the same inputs. Return only one ```python code block.\
"""
...
def score(source, data, ref, params):
    ...
    chem = qmlf.create_advanced_chem_layer(num_atoms=len(coordinates))
    want_desc = chem.molecular_descriptor(coordinates, charges); want_H = chem.build_hamiltonian(coordinates, charges)
    result, error = _run_model_function(source, "describe", (coordinates, charges))
    d_ok = h_ok = sym_ok = False
    if error is None:
        try:
            desc = np.asarray(result["descriptor"], dtype=float).ravel(); H = np.asarray(result["hamiltonian"], dtype=float)
            d_ok = desc.shape == want_desc.shape and np.allclose(desc, want_desc, atol=1e-9)
            h_ok = H.shape == want_H.shape and np.allclose(H, want_H, atol=1e-9); sym_ok = H.shape == want_H.shape and np.allclose(H, H.T)
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (d_ok, "descriptor must be the charge-aware Coulomb-matrix eigenvalue spectrum, sorted descending (qmlf.molecular_descriptor with charges)"),
              (h_ok and sym_ok, "hamiltonian must be qmlf's symmetric geometry-derived model Hamiltonian with the given charges")]
    return {"descriptor_ok": d_ok, "hamiltonian_ok": h_ok, "error": error}, checks



@kbench.task(name="qmlfb-chemistry-5atoms", description="Produce qmlf's charge-aware Coulomb descriptor and model Hamiltonian for a geometry.")
def qmlfb_chemistry_5atoms(llm) -> float:
    _ensure_qmlf()
    data = make_geometry(seed=11, n_atoms=5)
...
# %%
qmlfb_chemistry_5atoms.run(kbench.llm)
