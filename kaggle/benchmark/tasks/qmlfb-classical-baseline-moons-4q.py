...
Standard classical models: 0.90-0.97; auto quantum: 0.97.
"""
...
# %%
REF = {'c_min': 0.9, 'c_max': 0.9666666666666667, 'q_auto': 0.9666666666666667, 'c_bar': 0.82, 'q_bar': 0.89, 'keep': True}
PARAMS = {'kind': 'moons', 'n_features': 4}



PROMPT = """\
Professional QML practice demands a competent classical baseline next to every quantum model, honest estimates, and a verdict that follows from them. Dataset: 4 features, binary. `qmlf` is installed (qmlf.QuantumClassifier() auto-tunes; after fit, its CV sweep is the dict .cv_results_ with lists under keys 'bandwidth' and 'mean_cv_accuracy' -- max of the latter is its own CV estimate); sklearn is available. Return predictions as integer class labels. Write
...
@kbench.task(name="qmlfb-classical-baseline-moons-4q", description='Build a competent classical baseline AND a tuned quantum model; report calibrated estimates and a consistent verdict.')
def qmlfb_classical_baseline_moons_4q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("moons", n_samples=120, n_features=4, seed=71, class_sep=1.0)
...
# %%
qmlfb_classical_baseline_moons_4q.run(kbench.llm)
