# %%
"""Tier B -- Classical-baseline honesty (Bowles, Ahmed & Schuld 2024)

Standard classical models: 0.93-0.97; auto quantum: 0.93.
"""
...
# %%
REF = {'c_min': 0.9333333333333333, 'c_max': 0.9666666666666667, 'q_auto': 0.9333333333333333, 'c_bar': 0.85, 'q_bar': 0.85, 'keep': True}
PARAMS = {'kind': 'shift', 'n_features': 6}



PROMPT = """\
Professional QML practice demands a competent classical baseline next to every quantum model, honest estimates, and a verdict that follows from them. Dataset: 6 features, binary. `qmlf` is installed (qmlf.QuantumClassifier() auto-tunes; after fit, its CV sweep is the dict .cv_results_ with lists under keys 'bandwidth' and 'mean_cv_accuracy' -- max of the latter is its own CV estimate); sklearn is available. Return predictions as integer class labels. Write
    def compare(X_train, y_train, X_test):
        return {"classical_predictions": ..., "quantum_predictions": ..., "classical_estimate": <honest held-out estimate, e.g. CV>, "quantum_estimate": <same>, "winner": "classical" | "quantum" | "tie"}
Rules: winner must follow your own estimates ("tie" within 2 points); estimates must not overstate unseen-data performance; both models are scored (sandbagged baseline or untuned quantum fails). Return only one ```python code block.\
"""
...
def score(source, data, ref, params):
    ...
    if error is None:
        try:
            ...
            c_est, q_est = float(result["classical_estimate"]), float(result["quantum_estimate"]); winner = result["winner"]
            diff = c_est - q_est; expected = "tie" if abs(diff) < 0.02 else ("classical" if diff > 0 else "quantum")
    ...
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (c_acc >= ref["c_bar"], f"Classical baseline must be competent (>= {ref['c_bar']}; standard models score {ref['c_min']:.2f}-{ref['c_max']:.2f} here). Got {c_acc:.3f}"),
              (q_acc >= ref["q_bar"], f"Quantum model must be tuned (>= {ref['q_bar']}; auto gives {ref['q_auto']:.3f}). Got {q_acc:.3f}"),
              (consistent, f"Declared winner {winner!r} must follow the model's own estimates ({c_est}, {q_est})"),
              (calibrated, f"Estimates must not overstate held-out reality by >12 points (estimates {c_est}/{q_est} vs test {c_acc:.3f}/{q_acc:.3f})")]
    return {"classical_accuracy": c_acc, "quantum_accuracy": q_acc, "declared_winner": winner, "error": error}, checks



@kbench.task(name="qmlfb-classical-baseline-shift-6q", description='Build a competent classical baseline AND a tuned quantum model; report calibrated estimates and a consistent verdict.')
def qmlfb_classical_baseline_shift_6q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("shift", n_samples=120, n_features=6, seed=72, class_sep=1.0)
...
# %%
qmlfb_classical_baseline_shift_6q.run(kbench.llm)
