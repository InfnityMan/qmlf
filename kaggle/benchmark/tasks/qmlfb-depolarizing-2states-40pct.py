...
Raw L1 0.224; exact inversion recovers the ideal.
"""
...
# %%
REF = {'raw': 0.2238606417693039, 'keep': True}
PARAMS = {'n_states': 2, 'p': 0.4}
...
A circuit's output distribution passed through a global depolarizing channel with KNOWN probability p: noisy = (1 - p) * ideal + p * uniform. qmlf.create_advanced_noise_mitigator(strategy="depolarizing", noise_level=p).mitigate(noisy) inverts it exactly (it returns a 2-D (1, n_states) array for 1-D input; note noise_level must be in [0, 1)). Write
    def invert(noisy, p):
        return <length-2 ideal distribution>
...
@kbench.task(name="qmlfb-depolarizing-2states-40pct", description='Recover the ideal distribution from a known global depolarizing channel.')
def qmlfb_depolarizing_2states_40pct(llm) -> float:
    _ensure_qmlf()
    data = (lambda r: (lambda ideal: (ideal, (1 - 0.4) * ideal + 0.4 / 2, 0.4))(r.dirichlet(__import__('numpy').ones(2))))(__import__('numpy').random.default_rng(3))
...
# %%
qmlfb_depolarizing_2states_40pct.run(kbench.llm)
