...
Raw L1 0.097; exact inversion recovers the ideal.
"""
...
# %%
REF = {'raw': 0.09743541760878005, 'keep': True}
PARAMS = {'n_states': 4, 'p': 0.1}
...
A circuit's output distribution passed through a global depolarizing channel with KNOWN probability p: noisy = (1 - p) * ideal + p * uniform. qmlf.create_advanced_noise_mitigator(strategy="depolarizing", noise_level=p).mitigate(noisy) inverts it exactly (it returns a 2-D (1, n_states) array for 1-D input; note noise_level must be in [0, 1)). Write
    def invert(noisy, p):
        return <length-4 ideal distribution>
...
@kbench.task(name="qmlfb-depolarizing-4states-10pct", description='Recover the ideal distribution from a known global depolarizing channel.')
def qmlfb_depolarizing_4states_10pct(llm) -> float:
    _ensure_qmlf()
    data = (lambda r: (lambda ideal: (ideal, (1 - 0.1) * ideal + 0.1 / 4, 0.1))(r.dirichlet(__import__('numpy').ones(4))))(__import__('numpy').random.default_rng(0))
...
# %%
qmlfb_depolarizing_4states_10pct.run(kbench.llm)
