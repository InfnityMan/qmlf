# %%
"""Tier D -- Exact inversion of a known depolarizing channel

Raw L1 0.231; exact inversion recovers the ideal.
"""
...
# %%
REF = {'raw': 0.23061219167304378, 'keep': True}
PARAMS = {'n_states': 8, 'p': 0.25}



PROMPT = """\
A circuit's output distribution passed through a global depolarizing channel with KNOWN probability p: noisy = (1 - p) * ideal + p * uniform. qmlf.create_advanced_noise_mitigator(strategy="depolarizing", noise_level=p).mitigate(noisy) inverts it exactly (it returns a 2-D (1, n_states) array for 1-D input; note noise_level must be in [0, 1)). Write
    def invert(noisy, p):
        return <length-8 ideal distribution>
Scoring: L1 to the true ideal <= 1e-6. Return only one ```python code block.\
"""
...
def score(source, data, ref, params):
    import numpy as np
    ideal, noisy, p = data
    result, error = _run_model_function(source, "invert", (noisy, p))
    l1, valid = float("inf"), False
    if error is None:
        try:
            est = np.asarray(result, dtype=float).ravel(); valid = est.shape == ideal.shape and abs(est.sum() - 1) < 1e-6
            l1 = float(np.abs(est - ideal).sum())
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (valid, "Result must be a normalised distribution of the right length"),
              (l1 <= 1e-6, f"Exact inversion of a known global depolarizing channel recovers the ideal to machine precision (raw L1 {ref['raw']:.3f}). Got {l1:.2e}")]
    return {"l1_to_ideal": l1, "error": error}, checks



@kbench.task(name="qmlfb-depolarizing-8states-25pct", description='Recover the ideal distribution from a known global depolarizing channel.')
def qmlfb_depolarizing_8states_25pct(llm) -> float:
    _ensure_qmlf()
    data = (lambda r: (lambda ideal: (ideal, (1 - 0.25) * ideal + 0.25 / 8, 0.25))(r.dirichlet(__import__('numpy').ones(8))))(__import__('numpy').random.default_rng(1))
...
# %%
qmlfb_depolarizing_8states_25pct.run(kbench.llm)
