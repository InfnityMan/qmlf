# %%
"""Tier E -- Reproducibility audit

Two runs of the selector must produce identical feature importances (use random_state=...)
"""
...
# %%
REF = {'keep': True}
PARAMS = {'which': 'qiga', 'expectation': 'Two runs of the selector must produce identical feature importances (use random_state=...)'}



PROMPT = """\
Run qmlf's quantum-inspired genetic feature selector (qmlf.create_advanced_qiga_selector(population_size=6, generations=3, random_state=...)) on the training data and return its feature_importances_ array (the final per-feature selection probabilities). The grader calls your function twice and requires bit-identical results WITHOUT any global numpy seeding -- the selector must be made self-contained. Write
...
def score(source, data, ref, params):
    ...
    a, error = _run_model_function(source, "run", (X_train, y_train)); b = None
    if error is None: b, error = _run_model_function(source, "run", (X_train, y_train))
    same = False; val = None
    if error is None:
        try:
            va, vb = np.asarray(a, dtype=float), np.asarray(b, dtype=float); same = va.shape == vb.shape and np.array_equal(va, vb); val = va.ravel()[:4].tolist()
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run (twice). Got: {error}"),
              (same, params["expectation"])]
    return {"reproducible": same, "sample": val, "error": error}, checks



@kbench.task(name="qmlfb-determinism-feature-selector", description='Make a stochastic qmlf component bit-reproducible using its determinism knob.')
def qmlfb_determinism_feature_selector(llm) -> float:
    _ensure_qmlf(with_torch=True)
    data = make_dataset("blobs", n_samples=40, n_features=6, seed=101)
...
# %%
qmlfb_determinism_feature_selector.run(kbench.llm)
