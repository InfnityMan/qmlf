...
Two VQE runs must return identical energies (fix initial_point)
"""
...
# %%
REF = {'keep': True}
PARAMS = {'which': 'vqe', 'expectation': 'Two VQE runs must return identical energies (fix initial_point)'}



PROMPT = """\
Return qmlf's VQE energy for a 3-atom geometry built from the first three training rows as coordinates (coordinates = X_train[:3, :3]) with unit charges, via qmlf.create_advanced_chem_layer(num_atoms=3).compute_vqe_energy(...). The grader calls twice and requires identical floats; the default random initial point will not do. Write
...
@kbench.task(name="qmlfb-determinism-vqe-energy", description='Make a stochastic qmlf component bit-reproducible using its determinism knob.')
def qmlfb_determinism_vqe_energy(llm) -> float:
...
# %%
qmlfb_determinism_vqe_energy.run(kbench.llm)
