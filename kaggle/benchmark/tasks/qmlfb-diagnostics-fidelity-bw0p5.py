# %%
"""Tier A -- Kernel health diagnostics

fidelity kernel, bandwidth 0.5: offdiag 0.0340, KTA 0.1562, verdict severely concentrated.
"""
...
# %%
REF = {'offdiag': 0.0339838512305293, 'kta': 0.1561921802127098, 'verdict': 'severely concentrated', 'keep': True}
PARAMS = {'kernel': 'fidelity', 'bandwidth': 0.5}



PROMPT = """\
Audit a quantum kernel before deployment. Build qmlf.QuantumKernel(kernel="fidelity", bandwidth=0.5) fitted on X_train (.fit(X_train)), compute its training Gram matrix with kernel.compute_kernel_matrix(X_train), and report qmlf.kernel_diagnostics(gram, y_train) -- a dict with 'offdiag_mean', 'top_eigenvalue_fraction', 'kernel_target_alignment' and 'verdict' ('severely concentrated' / 'concentrated' / 'healthy'). Write
    def diagnose(X_train, y_train):
        return {"offdiag_mean": <float>, "kernel_target_alignment": <float>, "verdict": <str>}
Values are checked to 1e-6 against the library on the same data. Return only one ```python code block.\
"""
...
def score(source, data, ref, params):
    ...
    result, error = _run_model_function(source, "diagnose", (X_train, y_train))
    off_ok = verdict_ok = kta_ok = False; got = {}
    if error is None:
        try:
            got = dict(result); off = float(got["offdiag_mean"]); kta = float(got["kernel_target_alignment"])
            off_ok = abs(off - ref["offdiag"]) <= 1e-6; kta_ok = abs(kta - ref["kta"]) <= 1e-6; verdict_ok = got["verdict"] == ref["verdict"]
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (off_ok, f"offdiag_mean must be {ref['offdiag']:.6f} (the Gram matrix of the specified kernel on X_train)"),
              (kta_ok, f"kernel_target_alignment must be {ref['kta']:.6f}"),
              (verdict_ok, f"verdict must be {ref['verdict']!r}")]
    return {"got": got, "error": error}, checks



@kbench.task(name="qmlfb-diagnostics-fidelity-bw0p5", description='Report the exact concentration/alignment diagnostics of a specified quantum kernel.')
def qmlfb_diagnostics_fidelity_bw0p5(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=60, n_features=5, seed=113)
...
# %%
qmlfb_diagnostics_fidelity_bw0p5.run(kbench.llm)
