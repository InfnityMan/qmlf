...
'z' map 0.96 with 0 two-qubit gates; 'zz' map 0.92 with 40.
"""
...
# %%
REF = {'z_acc': 0.96, 'zz_acc': 0.92, 'zz_gates': 40, 'acc_bar': 0.92, 'keep': True}
PARAMS = {'kind': 'blobs', 'n_features': 5}
...
@kbench.task(name="qmlfb-entanglement-blobs-5q-seed5", description='Reach the accuracy bar with the minimum number of entangling gates.')
def qmlfb_entanglement_blobs_5q_seed5(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=100, n_features=5, seed=5, class_sep=1.3)
...
# %%
qmlfb_entanglement_blobs_5q_seed5.run(kbench.llm)
