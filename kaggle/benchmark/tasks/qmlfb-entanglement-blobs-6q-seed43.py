...
'z' map 0.96 with 0 two-qubit gates; 'zz' map 0.88 with 60.
"""
...
# %%
REF = {'z_acc': 0.96, 'zz_acc': 0.88, 'zz_gates': 60, 'acc_bar': 0.92, 'keep': True}
...
PROMPT = """\
Deploying a quantum-kernel classifier on hardware where every two-qubit entangling gate is the dominant error/cost source. Dataset: 6 features, binary, ~75/25 split. qmlf.QuantumClassifier(feature_map=..., entanglement=..., bandwidth=...) supports feature_map 'zz' (pairwise entangling gates) and 'z' (product encoding, zero entangling gates). Bowles, Ahmed & Schuld (2024) found removing entanglement often matches or improves accuracy on small tabular tasks. Objective: test accuracy >= 0.92 using the FEWEST entangling gates; you are scored on both. Choose the configuration.\
"""
...
@kbench.task(name="qmlfb-entanglement-blobs-6q-seed43", description='Reach the accuracy bar with the minimum number of entangling gates.')
def qmlfb_entanglement_blobs_6q_seed43(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=100, n_features=6, seed=43, class_sep=1.3)
...
# %%
qmlfb_entanglement_blobs_6q_seed43.run(kbench.llm)
