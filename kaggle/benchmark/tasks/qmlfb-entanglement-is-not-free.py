...
Bowles et al. found removing entanglement often helps. On this dataset the
unentangled 'z' map scores 0.96 with 0 two-qubit gates; the
entangled 'zz' map scores 0.92 with 40.
...
@kbench.task(name="qmlfb-entanglement-is-not-free",
...
def qmlf_entanglement_is_not_free(llm) -> dict:
    ...
    kbench.assertions.assert_true(gates == 0, expectation=f"The bar is reachable with zero entangling gates ('z' map scores 0.96); used {gates}")
    return _jsonable({"feature_map": plan.feature_map, "accuracy": acc, "entangling_gates": gates, "error": error})


# %%
qmlf_entanglement_is_not_free.run(kbench.llm)
