# %%
"""Tier A -- Entanglement is not free (Bowles, Ahmed & Schuld 2024)

'z' map 0.88 with 0 two-qubit gates; 'zz' map 0.84 with 40.
"""
...
# %%
REF = {'z_acc': 0.88, 'zz_acc': 0.84, 'zz_gates': 40, 'acc_bar': 0.84, 'keep': True}
PARAMS = {'kind': 'shift', 'n_features': 5}


class Plan(BaseModel):
    feature_map: str = Field(description="'zz' (entangled) or 'z' (product encoding, zero entangling gates)")
    entanglement: str = Field(description="'full', 'linear' or 'circular' (ignored for 'z')")
    bandwidth: str = Field(description="'auto' or a positive number as a string")
    rationale: str = Field(description="Why this reaches the bar at minimum hardware cost")


PROMPT = """\
Deploying a quantum-kernel classifier on hardware where every two-qubit entangling gate is the dominant error/cost source. Dataset: 5 features, binary, ~75/25 split. qmlf.QuantumClassifier(feature_map=..., entanglement=..., bandwidth=...) supports feature_map 'zz' (pairwise entangling gates) and 'z' (product encoding, zero entangling gates). Bowles, Ahmed & Schuld (2024) found removing entanglement often matches or improves accuracy on small tabular tasks. Objective: test accuracy >= 0.84 using the FEWEST entangling gates; you are scored on both. Choose the configuration.\
"""
...
def score(plan, data, ref, params):
    ...
    try:
        ...
        if plan.feature_map == "zz": kwargs["entanglement"] = plan.entanglement
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            clf = qmlf.QuantumClassifier(**kwargs).fit(X_train, y_train)
        acc = float(clf.score(X_test, y_test)); gates = int(clf._kernel.feature_map.num_nonlocal_gates())
    except Exception as exc:
        error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Configuration must be valid. Got: {error}"),
              (acc >= ref["acc_bar"], f"Accuracy >= {ref['acc_bar']}. Got {acc:.3f}"),
              (gates == 0, f"The bar is reachable with zero entangling gates ('z' scores {ref['z_acc']:.2f}); used {gates}")]
    return {"accuracy": acc, "entangling_gates": gates, "plan": plan.model_dump(), "error": error}, checks



@kbench.task(name="qmlfb-entanglement-shift-5q-seed42", description='Reach the accuracy bar with the minimum number of entangling gates.')
def qmlfb_entanglement_shift_5q_seed42(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("shift", n_samples=100, n_features=5, seed=42, class_sep=1.3)
...
# %%
qmlfb_entanglement_shift_5q_seed42.run(kbench.llm)
