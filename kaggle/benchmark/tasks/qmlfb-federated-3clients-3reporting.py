...
3 of 3 clients reported with unequal sample counts.
"""
...
# %%
REF = {'expected': [-0.8899102363595237, 1.067171819953865, 0.44712575374372915, -0.18393070121554866], 'unweighted': [-0.8437345269850726, 0.9587664437836727, 0.12477616563399803, -0.27492188655540345], 'keep': True}
PARAMS = {'n_clients': 3, 'n_reporting': 3, 'dim': 4}



PROMPT = """\
A federated quantum-ML round: 3 clients were expected but only 3 returned parameter vectors (shape (3, 4)), trained on the given local sample counts. qmlf.create_federated_qml(num_clients=...) returns an aggregator with .aggregate(client_params_list, client_weights=None) (sample-size-weighted mean when weights are given; it warns by design when fewer clients than num_clients report). Write
    def aggregate(client_params, sample_counts, num_clients):
        return <length-4 global parameter vector>
...
@kbench.task(name="qmlfb-federated-3clients-3reporting", description='Produce the exact sample-weighted FedAvg update under partial client participation.')
def qmlfb_federated_3clients_3reporting(llm) -> float:
    _ensure_qmlf()
    data = make_federated(seed=43, n_clients=3, n_reporting=3, dim=4)
...
# %%
qmlfb_federated_3clients_3reporting.run(kbench.llm)
