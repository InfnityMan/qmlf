...
3 of 5 clients reported with unequal sample counts.
"""
...
# %%
REF = {'expected': [-0.10300066931388156, -0.47827251763106304, -0.6360927313552804, 0.3484795981494535, -0.40683623847953243, 0.5218235852540569], 'unweighted': [-0.08112544725947908, -0.4825823153421398, -0.6372514808351853, 0.3748530035829194, -0.3805183265641226, 0.4966663149940643], 'keep': True}
PARAMS = {'n_clients': 5, 'n_reporting': 3, 'dim': 6}



PROMPT = """\
A federated quantum-ML round: 5 clients were expected but only 3 returned parameter vectors (shape (3, 6)), trained on the given local sample counts. qmlf.create_federated_qml(num_clients=...) returns an aggregator with .aggregate(client_params_list, client_weights=None) (sample-size-weighted mean when weights are given; it warns by design when fewer clients than num_clients report). Write
...
@kbench.task(name="qmlfb-federated-5clients-3reporting", description='Produce the exact sample-weighted FedAvg update under partial client participation.')
def qmlfb_federated_5clients_3reporting(llm) -> float:
    _ensure_qmlf()
    data = make_federated(seed=41, n_clients=5, n_reporting=3, dim=6)
...
# %%
qmlfb_federated_5clients_3reporting.run(kbench.llm)
