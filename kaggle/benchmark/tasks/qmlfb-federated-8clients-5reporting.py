# %%
"""Tier F -- Federated QML round with partial participation

5 of 8 clients reported with unequal sample counts.
"""
...
# %%
REF = {'expected': [0.18838315291505484, -0.3000324086200618, 0.17811130539801148, 0.4164293917775102, -0.571906244255366, -0.4706537886444229, 0.043703995533396545, -0.1887293811275294, -0.09069688802636552, -0.06277684554881229, 0.581536733187809, 0.50164631803121], 'unweighted': [0.10147721211391199, -0.2074907714468658, 0.24298158132211922, 0.3457194663070591, -0.37669068832551467, -0.3213516105471082, 0.40241118875270593, -0.23586000897821452, -0.17447253844140845, -0.12682325845239806, 0.5447032796734602, 0.5888217298894401], 'keep': True}
PARAMS = {'n_clients': 8, 'n_reporting': 5, 'dim': 12}



PROMPT = """\
A federated quantum-ML round: 8 clients were expected but only 5 returned parameter vectors (shape (5, 12)), trained on the given local sample counts. qmlf.create_federated_qml(num_clients=...) returns an aggregator with .aggregate(client_params_list, client_weights=None) (sample-size-weighted mean when weights are given; it warns by design when fewer clients than num_clients report). Write
    def aggregate(client_params, sample_counts, num_clients):
        return <length-12 global parameter vector>
...
def score(source, data, ref, params):
    import numpy as np
    client_params, counts, n_clients = data
    expected = np.asarray(ref["expected"]); unweighted = np.asarray(ref["unweighted"])
    result, error = _run_model_function(source, "aggregate", (client_params, counts, n_clients))
    ok = is_unw = False
    if error is None:
        try:
            v = np.asarray(result, dtype=float).ravel(); ok = v.shape == expected.shape and np.allclose(v, expected, atol=1e-9); is_unw = v.shape == expected.shape and np.allclose(v, unweighted, atol=1e-9)
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (not is_unw, "Unweighted mean is the classic FedAvg mistake; weight by sample count"),
              (ok, "Global vector must equal the sample-weighted mean to 1e-9")]
    return {"correct": ok, "unweighted_mistake": is_unw, "error": error}, checks



@kbench.task(name="qmlfb-federated-8clients-5reporting", description='Produce the exact sample-weighted FedAvg update under partial client participation.')
def qmlfb_federated_8clients_5reporting(llm) -> float:
    _ensure_qmlf()
    data = make_federated(seed=42, n_clients=8, n_reporting=5, dim=12)
...
# %%
qmlfb_federated_8clients_5reporting.run(kbench.llm)
