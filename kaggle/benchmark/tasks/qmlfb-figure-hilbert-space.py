# %%
"""Tier F -- Headless research figure

QVizPro.plot_hilbert_space(gram, labels=y_train, ...) written to HTML, no display.
"""
...
# %%
REF = {'keep': True}
PARAMS = {'which': 'hilbert'}



PROMPT = """\
Produce a research figure of a quantum kernel in a HEADLESS sandbox (no display; anything that tries to open a browser fails). Compute the training Gram matrix gram = qmlf.QuantumKernel(bandwidth=0.1).fit(X_train).compute_kernel_matrix(X_train), then use qmlf.viz.q_viz_pro.QVizPro -- specifically QVizPro.plot_hilbert_space(gram, labels=y_train, ...) -- which takes show= and save_path= keyword arguments. Write
...
def score(source, data, ref, params):
    ...
    out = os.path.join(tempfile.mkdtemp(), "figure.html")
    result, error = _run_model_function(source, "make_figure", (X_train, y_train, out))
    exists = os.path.exists(out) and os.path.getsize(out) > 1000; is_fig = False
    if error is None:
        try:
            import plotly.graph_objects as go
            is_fig = isinstance(result, go.Figure)
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run headless (no display; fig.show() must not be called). Got: {error}"),
              (is_fig, "Must return the plotly Figure"),
              (exists, "Must write the figure to the given HTML path (save_path=...)")]
    return {"file_written": exists, "error": error}, checks



@kbench.task(name="qmlfb-figure-hilbert-space", description='Produce a qmlf research figure in a headless sandbox and write it to disk.')
def qmlfb_figure_hilbert_space(llm) -> float:
...
# %%
qmlfb_figure_hilbert_space.run(kbench.llm)
