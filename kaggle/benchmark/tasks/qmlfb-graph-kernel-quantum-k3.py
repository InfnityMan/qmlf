...
quantum backend, k=3, n_qubits=4.
"""
...
@kbench.task(name="qmlfb-graph-kernel-quantum-k3", description="Build qmlf's graph-diffusion kernel with the specified backend and neighbourhood, including out-of-sample similarity.")
def qmlfb_graph_kernel_quantum_k3(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=32, n_features=4, seed=122)
...
# %%
qmlfb_graph_kernel_quantum_k3.run(kbench.llm)
