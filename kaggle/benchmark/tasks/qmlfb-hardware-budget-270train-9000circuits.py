# %%
"""Tier C -- Hardware circuit budget via Nystrom landmarks

270 rows need 36,315 pairwise circuits; budget 9,000; max feasible m=31 gives 0.867.
"""
...
# %%
REF = {'pairwise': 36315, 'm_max': 31, 'acc_mmax': 0.8666666666666667, 'acc_small': 0.7, 'acc_bar': 0.81, 'keep': True}
PARAMS = {'budget': 9000, 'n_features': 4, 'n_train': 270}



PROMPT = """\
Train a quantum-kernel classifier on hardware. The full Gram matrix for n training samples costs n(n-1)/2 fidelity circuits; your HARD budget is 9,000 circuits. Dataset: 4 features, binary, 270 train rows. A fitted qmlf.QuantumKernel exposes circuit_budget(n_samples, n_landmarks=m) -> dict with 'nystrom_fidelity_circuits', and nystrom_features(X, n_landmarks=m) -> (n, m) explicit features costing n*m + m(m-1)/2 circuits. Write
    def solve(X_train, y_train, budget=9000):
        return {"kernel": <qmlf.QuantumKernel fitted on X_train with a sensible bandwidth>, "n_landmarks": <int m respecting the budget>}
The grader builds Nystrom features from YOUR kernel and m (same landmarks on train/test), trains a linear SVM, and checks: budget respected AND accuracy >= 0.81. Maximise m within the budget. Return only one ```python code block.\
"""


def score(source, data, ref, params):
    import numpy as np, qmlf
    ...
    if error is None:
        try:
            kernel = result["kernel"]; m = int(result["n_landmarks"]); ok_type = isinstance(kernel, qmlf.QuantumKernel)
            n = len(X_train); cost = int(kernel.circuit_budget(n, m)["nystrom_fidelity_circuits"])
            idx = np.unique(np.linspace(0, n - 1, m).round().astype(int)); L = X_train[idx]
            W = kernel.compute_kernel_matrix(L); ev, evec = np.linalg.eigh((W + W.T) / 2)
            inv = np.where(ev > 1e-12, 1 / np.sqrt(np.maximum(ev, 1e-12)), 0.0); proj = (evec * inv) @ evec.T
            F_tr = kernel.compute_kernel_matrix(X_train, L) @ proj; F_te = kernel.compute_kernel_matrix(X_test, L) @ proj
            acc = float(LinearSVC(dual="auto").fit(F_tr, y_train).score(F_te, y_test))
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (ok_type, "Must return a fitted qmlf.QuantumKernel"),
              (cost is not None and cost <= budget, f"Nystrom cost must be <= {budget} circuits (pairwise needs {ref['pairwise']:,}). Got {cost} with m={m}"),
              (acc >= ref["acc_bar"], f"Accuracy >= {ref['acc_bar']} (m={ref['m_max']} gives {ref['acc_mmax']:.3f}). Got {acc:.3f}")]
    return {"n_landmarks": m, "circuits": cost, "accuracy": acc, "error": error}, checks



@kbench.task(name="qmlfb-hardware-budget-270train-9000circuits", description='Stay inside a hardware circuit budget with Nystrom landmarks while keeping accuracy.')
def qmlfb_hardware_budget_270train_9000circuits(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=360, n_features=4, seed=83, class_sep=1.4)
...
# %%
qmlfb_hardware_budget_270train_9000circuits.run(kbench.llm)
