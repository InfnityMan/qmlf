...
300 rows need 44,850 pairwise circuits; budget 25,000; max feasible m=74 gives 0.760.
"""
...
# %%
REF = {'pairwise': 44850, 'm_max': 74, 'acc_mmax': 0.76, 'acc_small': 0.6, 'acc_bar': 0.7, 'keep': True}
PARAMS = {'budget': 25000, 'n_features': 6, 'n_train': 300}



PROMPT = """\
Train a quantum-kernel classifier on hardware. The full Gram matrix for n training samples costs n(n-1)/2 fidelity circuits; your HARD budget is 25,000 circuits. Dataset: 6 features, binary, 300 train rows. A fitted qmlf.QuantumKernel exposes circuit_budget(n_samples, n_landmarks=m) -> dict with 'nystrom_fidelity_circuits', and nystrom_features(X, n_landmarks=m) -> (n, m) explicit features costing n*m + m(m-1)/2 circuits. Write
...
The grader builds Nystrom features from YOUR kernel and m (same landmarks on train/test), trains a linear SVM, and checks: budget respected AND accuracy >= 0.7. Maximise m within the budget. Return only one ```python code block.\
"""
...
@kbench.task(name="qmlfb-hardware-budget-300train-25000circuits", description='Stay inside a hardware circuit budget with Nystrom landmarks while keeping accuracy.')
def qmlfb_hardware_budget_300train_25000circuits(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=400, n_features=6, seed=17, class_sep=1.4)
...
# %%
qmlfb_hardware_budget_300train_25000circuits.run(kbench.llm)
