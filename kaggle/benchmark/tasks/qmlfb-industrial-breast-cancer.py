# %%
"""Tier F -- Industrial pipeline on real tabular data

breast_cancer: naive 0.62, auto 0.92, projected 0.88, classical RBF 0.98.
"""
...
# %%
REF = {'naive_acc': 0.62, 'auto_acc': 0.92, 'projected_acc': 0.88, 'classical': 0.98, 'acc_bar': 0.86, 'keep': True}
PARAMS = {'name': 'breast_cancer', 'label': 'breast-cancer diagnostics', 'n_features': 30, 'classes': 'binary'}
...
class Plan(BaseModel):
    ...
    max_qubits: int = Field(description="Qubit cap; wider data is PCA-reduced inside fit")
...
PROMPT = """\
Production task on the breast-cancer diagnostics dataset (30 standardised features, binary). Configure qmlf.QuantumClassifier for deployment: kernel ('fidelity'|'projected'|'auto'), mode, feature_map, bandwidth ('auto' = deterministic CV sweep, 'median', or a number), max_qubits (features beyond it are PCA-reduced inside fit; exact statevector cost doubles per qubit), normalize. Scoring: test accuracy >= 0.86 AND the fitted kernel must not be 'severely concentrated' per clf.diagnose(). A competent classical RBF reaches ~0.98 here -- expect a correct, healthy pipeline, not a quantum win. Choose the configuration.\
"""
...
def score(plan, data, ref, params):
    ...
    try:
        bw = plan.bandwidth.strip().lower(); bandwidth = bw if bw in ("auto", "median") else float(bw)
        normalize = None if plan.normalize in ("default", "") else plan.normalize
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            clf = qmlf.QuantumClassifier(kernel=plan.kernel, mode=plan.mode, feature_map=plan.feature_map, bandwidth=bandwidth,
                                         max_qubits=int(plan.max_qubits), normalize=normalize).fit(X_train, y_train)
        acc = float(clf.score(X_test, y_test)); verdict = clf.diagnose()["verdict"]
    except Exception as exc:
        error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Configuration must be valid. Got: {error}"),
              (verdict is not None and verdict != "severely concentrated", f"Fitted kernel must not be severely concentrated. Got {verdict!r}"),
              (acc >= ref["acc_bar"], f"Accuracy >= {ref['acc_bar']} (naive {ref['naive_acc']:.2f}, auto {ref['auto_acc']:.2f}, classical RBF {ref['classical']:.2f}). Got {acc:.3f}")]
    return {"accuracy": acc, "verdict": verdict, "plan": plan.model_dump(), "error": error}, checks



@kbench.task(name="qmlfb-industrial-breast-cancer", description='Configure a production quantum-kernel pipeline on real data: reduce, encode, tune.')
def qmlfb_industrial_breast_cancer(llm) -> float:
    _ensure_qmlf()
    data = make_real("breast_cancer")
...
# %%
qmlfb_industrial_breast_cancer.run(kbench.llm)
