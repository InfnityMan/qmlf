...
wine: naive 0.39, auto 0.95, projected 0.95, classical RBF 0.95.
"""
...
# %%
REF = {'naive_acc': 0.39473684210526316, 'auto_acc': 0.9473684210526315, 'projected_acc': 0.9473684210526315, 'classical': 0.9473684210526315, 'acc_bar': 0.89, 'keep': True}
PARAMS = {'name': 'wine', 'label': 'wine cultivar', 'n_features': 13, 'classes': '3 classes'}
...
PROMPT = """\
Production task on the wine cultivar dataset (13 standardised features, 3 classes). Configure qmlf.QuantumClassifier for deployment: kernel ('fidelity'|'projected'|'auto'), mode, feature_map, bandwidth ('auto' = deterministic CV sweep, 'median', or a number), max_qubits (features beyond it are PCA-reduced inside fit; exact statevector cost doubles per qubit), normalize. Scoring: test accuracy >= 0.89 AND the fitted kernel must not be 'severely concentrated' per clf.diagnose(). A competent classical RBF reaches ~0.95 here -- expect a correct, healthy pipeline, not a quantum win. Choose the configuration.\
"""
...
@kbench.task(name="qmlfb-industrial-wine", description='Configure a production quantum-kernel pipeline on real data: reduce, encode, tune.')
def qmlfb_industrial_wine(llm) -> float:
    _ensure_qmlf()
    data = make_real("wine")
...
# %%
qmlfb_industrial_wine.run(kbench.llm)
