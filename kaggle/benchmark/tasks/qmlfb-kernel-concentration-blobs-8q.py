...
Naive (bandwidth 1.0): acc 0.54, off-diagonal 0.0041. Tuned: 0.93 / 0.38. Data: make_dataset("blobs", n_samples=110, n_features=8, seed=2)
"""
...
# %%
REF = {'naive_acc': 0.5357142857142857, 'naive_offdiag': 0.004107810335006377, 'ceiling_acc': 0.9285714285714286, 'ceiling_offdiag': 0.3773217923850245, 'offdiag_bar': 0.1, 'acc_bar': 0.85, 'keep': True}
PARAMS = {'kind': 'blobs', 'n_features': 8}
...
PROMPT = """\
You are configuring qmlf.QuantumClassifier for a binary dataset with 8 continuous features (blobs structure). All features are encoded on 8 qubits (max_qubits=None). Fidelity kernels concentrate: with wide encoding angles the Gram matrix collapses toward the identity and the SVM memorises. Scoring (measured from the classifier the grader builds from your plan): training-Gram off-diagonal mean >= 0.1 AND test accuracy >= 0.85. Parameters: kernel, mode, bandwidth ('auto' runs a deterministic CV sweep; 'median'; 'alignment'; or a number), normalize, feature_map, entanglement. Return your configuration.\
"""
...
@kbench.task(name="qmlfb-kernel-concentration-blobs-8q", description='Configure a quantum kernel classifier that escapes concentration on this dataset.')
def qmlfb_kernel_concentration_blobs_8q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=110, n_features=8, seed=2)
...
# %%
qmlfb_kernel_concentration_blobs_8q.run(kbench.llm)
