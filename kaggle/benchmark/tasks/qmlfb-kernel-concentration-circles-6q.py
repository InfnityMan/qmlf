# %%
"""Tier A -- Fidelity-kernel concentration rescue

Naive (bandwidth 1.0): acc 0.71, off-diagonal 0.0204. Tuned: 0.96 / 0.36. Data: make_dataset("circles", n_samples=110, n_features=6, seed=4)
"""
...
# %%
REF = {'naive_acc': 0.7142857142857143, 'naive_offdiag': 0.020352216140901068, 'ceiling_acc': 0.9642857142857143, 'ceiling_offdiag': 0.3568787024748122, 'offdiag_bar': 0.1, 'acc_bar': 0.88, 'keep': True}
PARAMS = {'kind': 'circles', 'n_features': 6}


class Plan(BaseModel):
    kernel: str = Field(description="'fidelity' or 'projected'")
    mode: str = Field(description="'ZZ', 'covariant', 'mahalanobis' or 'fisher'")
    bandwidth: str = Field(description="'auto', 'median', 'alignment', or a positive number as a string")
...
PROMPT = """\
You are configuring qmlf.QuantumClassifier for a binary dataset with 6 continuous features (circles structure). All features are encoded on 6 qubits (max_qubits=None). Fidelity kernels concentrate: with wide encoding angles the Gram matrix collapses toward the identity and the SVM memorises. Scoring (measured from the classifier the grader builds from your plan): training-Gram off-diagonal mean >= 0.1 AND test accuracy >= 0.88. Parameters: kernel, mode, bandwidth ('auto' runs a deterministic CV sweep; 'median'; 'alignment'; or a number), normalize, feature_map, entanglement. Return your configuration.\
"""


def score(plan, data, ref, params):
    import warnings, qmlf
    X_train, X_test, y_train, y_test = data
    acc, offdiag, error = 0.0, 0.0, None
    try:
        bw = plan.bandwidth.strip().lower()
        bandwidth = bw if bw in ("auto", "median", "alignment") else float(bw)
        normalize = None if plan.normalize in ("default", "") else plan.normalize
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            clf = qmlf.QuantumClassifier(kernel=plan.kernel, mode=plan.mode, bandwidth=bandwidth,
                                         normalize=normalize, feature_map=plan.feature_map,
                                         entanglement=plan.entanglement, max_qubits=None).fit(X_train, y_train)
        acc = float(clf.score(X_test, y_test)); offdiag = float(clf.diagnose()["offdiag_mean"])
    except Exception as exc:
        error = f"{type(exc).__name__}: {exc}"
    checks = [
        (error is None, f"Configuration must be valid. Got: {error}"),
        (offdiag >= ref["offdiag_bar"], f"Gram off-diagonal must be >= {ref['offdiag_bar']} (naive {ref['naive_offdiag']:.4f}). Got {offdiag:.4f}"),
        (acc >= ref["acc_bar"], f"Accuracy must be >= {ref['acc_bar']} (naive {ref['naive_acc']:.2f}, tuned {ref['ceiling_acc']:.2f}). Got {acc:.3f}"),
    ]
    return {"accuracy": acc, "offdiag_mean": offdiag, "plan": plan.model_dump(), "error": error}, checks



@kbench.task(name="qmlfb-kernel-concentration-circles-6q", description='Configure a quantum kernel classifier that escapes concentration on this dataset.')
def qmlfb_kernel_concentration_circles_6q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("circles", n_samples=110, n_features=6, seed=4)
    answer = llm.prompt(PROMPT, schema=Plan)
    if False:
...
# %%
qmlfb_kernel_concentration_circles_6q.run(kbench.llm)
