...
Naive (bandwidth 1.0): acc 0.50, off-diagonal 0.0167. Tuned: 0.82 / 0.38. Data: make_dataset("quantiles", n_samples=110, n_features=6, seed=8)
"""
...
# %%
REF = {'naive_acc': 0.5, 'naive_offdiag': 0.016710207002881124, 'ceiling_acc': 0.8214285714285714, 'ceiling_offdiag': 0.3796574206934465, 'offdiag_bar': 0.1, 'acc_bar': 0.74, 'keep': True}
PARAMS = {'kind': 'quantiles', 'n_features': 6}
...
PROMPT = """\
You are configuring qmlf.QuantumClassifier for a binary dataset with 6 continuous features (quantiles structure). All features are encoded on 6 qubits (max_qubits=None). Fidelity kernels concentrate: with wide encoding angles the Gram matrix collapses toward the identity and the SVM memorises. Scoring (measured from the classifier the grader builds from your plan): training-Gram off-diagonal mean >= 0.1 AND test accuracy >= 0.74. Parameters: kernel, mode, bandwidth ('auto' runs a deterministic CV sweep; 'median'; 'alignment'; or a number), normalize, feature_map, entanglement. Return your configuration.\
"""
...
@kbench.task(name="qmlfb-kernel-concentration-quantiles-6q", description='Configure a quantum kernel classifier that escapes concentration on this dataset.')
def qmlfb_kernel_concentration_quantiles_6q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("quantiles", n_samples=110, n_features=6, seed=8)
...
# %%
qmlfb_kernel_concentration_quantiles_6q.run(kbench.llm)
