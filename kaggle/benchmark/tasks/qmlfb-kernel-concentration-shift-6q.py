...
Naive (bandwidth 1.0): acc 0.50, off-diagonal 0.0171. Tuned: 0.93 / 0.84. Data: make_dataset("shift", n_samples=110, n_features=6, seed=5)
"""
...
# %%
REF = {'naive_acc': 0.5, 'naive_offdiag': 0.01706731471600788, 'ceiling_acc': 0.9285714285714286, 'ceiling_offdiag': 0.8368214645646049, 'offdiag_bar': 0.1, 'acc_bar': 0.85, 'keep': True}
PARAMS = {'kind': 'shift', 'n_features': 6}
...
@kbench.task(name="qmlfb-kernel-concentration-shift-6q", description='Configure a quantum kernel classifier that escapes concentration on this dataset.')
def qmlfb_kernel_concentration_shift_6q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("shift", n_samples=110, n_features=6, seed=5)
...
# %%
qmlfb_kernel_concentration_shift_6q.run(kbench.llm)
