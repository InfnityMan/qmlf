# %%
"""Tier C -- Circuit-budget planning

n=1000, m=50: {'pairwise_fidelity_circuits': 499500, 'statevector_simulations': 1000, 'nystrom_fidelity_circuits': 51225}
"""
...
# %%
REF = {'expected': {'pairwise_fidelity_circuits': 499500, 'statevector_simulations': 1000, 'nystrom_fidelity_circuits': 51225}, 'keep': True}
PARAMS = {'n_samples': 1000, 'n_landmarks': 50}



PROMPT = """\
A hardware run is being planned for a quantum-kernel model on n training samples with m Nystrom landmarks. Report how many circuit evaluations each path needs: the pairwise fidelity path (one circuit per unordered sample pair), the exact statevector path (one simulation per sample), and the Nystrom path (n*m cross circuits plus the landmark block). qmlf.QuantumKernel(...).circuit_budget(n_samples, n_landmarks=m) implements exactly this accounting. Write
    def plan(n_samples, n_landmarks):
        return {"pairwise_fidelity_circuits": int, "statevector_simulations": int, "nystrom_fidelity_circuits": int}
Checked exactly. Return only one ```python code block.\
"""


def score(source, data, ref, params):
    result, error = _run_model_function(source, "plan", (params["n_samples"], params["n_landmarks"]))
    ok = False; got = None
    if error is None:
        try:
            got = {k: int(result[k]) for k in ("pairwise_fidelity_circuits", "statevector_simulations", "nystrom_fidelity_circuits")}
            ok = got == ref["expected"]
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run. Got: {error}"), (ok, f"Circuit budget must be exactly {ref['expected']}. Got {got}")]
    return {"got": got, "error": error}, checks



@kbench.task(name="qmlfb-nisq-planning-1000samples-50landmarks", description='Report the exact circuit counts of each kernel evaluation path for a planned hardware run.')
def qmlfb_nisq_planning_1000samples_50landmarks(llm) -> float:
    _ensure_qmlf()
    data = None
...
# %%
qmlfb_nisq_planning_1000samples_50landmarks.run(kbench.llm)
