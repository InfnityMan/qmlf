...
n=64, m=8: {'pairwise_fidelity_circuits': 2016, 'statevector_simulations': 64, 'nystrom_fidelity_circuits': 540}
"""
...
# %%
REF = {'expected': {'pairwise_fidelity_circuits': 2016, 'statevector_simulations': 64, 'nystrom_fidelity_circuits': 540}, 'keep': True}
PARAMS = {'n_samples': 64, 'n_landmarks': 8}
...
@kbench.task(name="qmlfb-nisq-planning-64samples-8landmarks", description='Report the exact circuit counts of each kernel evaluation path for a planned hardware run.')
def qmlfb_nisq_planning_64samples_8landmarks(llm) -> float:
    _ensure_qmlf()
    data = None
...
# %%
qmlfb_nisq_planning_64samples_8landmarks.run(kbench.llm)
