...
Planted: raw fidelity kernel handed to QSVC (discards whitening/normalize/bandwidth); readout mitigator used without .fit(confusion). Correct pipeline: acc 0.95, L1 0.0334.
"""
...
# %%
REF = {'correct_acc': 0.95, 'correct_l1': 0.033354719646998016, 'acc_bar': 0.88, 'l1_bar': 0.06, 'keep': True}
PARAMS = {'bugs': ['bypass', 'nofit']}
...
def run_pipeline(X_train, y_train, X_test, confusion, observed):
    ...
    kernel = qmlf.QuantumKernel(mode="mahalanobis", normalize="maxabs", bandwidth=0.1)
    kernel.fit(X_train)
...
@kbench.task(name="qmlfb-pipeline-repair-2bugs-a", description='Find and fix the planted bugs in a quantum kernel + mitigation pipeline.')
def qmlfb_pipeline_repair_2bugs_a(llm) -> float:
    _ensure_qmlf()
    data = (make_dataset("blobs", n_samples=80, n_features=5, seed=30, class_sep=1.4), make_mitigation(seed=1, n_states=4, base_p=0.12, readout_err=0.08, scales=(1, 2, 3)))
...
# %%
qmlfb_pipeline_repair_2bugs_a.run(kbench.llm)
