...
Planted: n_qubits disagrees with the 5-feature data; 1-D input to ZNE; whitened mode without normalisation (angles alias). Correct pipeline: acc 0.90, L1 0.0186.
"""
...
# %%
REF = {'correct_acc': 0.9, 'correct_l1': 0.018638546271066106, 'acc_bar': 0.83, 'l1_bar': 0.038, 'keep': True}
PARAMS = {'bugs': ['width', 'zne1d', 'normnone']}
...
def run_pipeline(X_train, y_train, X_test, confusion, observed):
    ...
    kernel = qmlf.QuantumKernel(n_qubits=8, mode="mahalanobis", normalize=None, bandwidth=0.1)
    kernel.fit(X_train)
    ...
    ro = qmlf.create_advanced_noise_mitigator(strategy="readout").fit(confusion)
...
@kbench.task(name="qmlfb-pipeline-repair-3bugs-a", description='Find and fix the planted bugs in a quantum kernel + mitigation pipeline.')
def qmlfb_pipeline_repair_3bugs_a(llm) -> float:
    _ensure_qmlf()
    data = (make_dataset("blobs", n_samples=80, n_features=5, seed=31, class_sep=1.4), make_mitigation(seed=2, n_states=4, base_p=0.12, readout_err=0.08, scales=(1, 2, 3)))
...
# %%
qmlfb_pipeline_repair_3bugs_a.run(kbench.llm)
