# %%
"""Tier F -- Repair a broken quantum pipeline

Planted: mis-cased strategy name; readout mitigator used without .fit(confusion); whitened mode without normalisation (angles alias). Correct pipeline: acc 0.90, L1 0.0539.
"""
...
# %%
REF = {'correct_acc': 0.9, 'correct_l1': 0.053924524316788194, 'acc_bar': 0.83, 'l1_bar': 0.0909, 'keep': True}
PARAMS = {'bugs': ['casing', 'nofit', 'normnone']}



PROMPT = """\
The following qmlf pipeline is broken in 3 places (it may raise, and where it does not raise it silently computes the wrong thing). Fix it, preserving the intent: a whitened ('mahalanobis') quantum kernel with maxabs normalisation and bandwidth 0.1 classifying 5-feature data with sklearn SVC(kernel='precomputed'), readout correction of each measured distribution using the confusion matrix, then zero-noise extrapolation across the three scales returning a length-4 distribution. qmlf facts: the feature map encodes one angle per feature; preprocessing lives in compute_kernel_matrix(X) / compute_kernel_matrix(X_test, X_train); strategy names are lower-case; 'readout' needs .fit(confusion) first; 'zne' needs the full (n_scales, n_states) stack; whitened modes need normalize='maxabs'.
...
def run_pipeline(X_train, y_train, X_test, confusion, observed):
    # X has 5 features; observed: distributions at noise scales 1, 2, 3 -> shape (3, 4)
    kernel = qmlf.QuantumKernel(mode="mahalanobis", normalize=None, bandwidth=0.1)
    kernel.fit(X_train)
    svc = SVC(kernel="precomputed").fit(kernel.compute_kernel_matrix(X_train), y_train)
    predictions = svc.predict(kernel.compute_kernel_matrix(X_test, X_train))
    ro = qmlf.create_advanced_noise_mitigator(strategy="readout")
    corrected = np.vstack([ro.mitigate(o) for o in observed])
    mitigator = qmlf.create_advanced_noise_mitigator(strategy="ZNE")
    mitigator.scale_factors = np.array([1.0, 2.0, 3.0])
    mitigated = mitigator.mitigate(corrected)
    return {"predictions": predictions, "mitigated": mitigated}


Return the complete corrected code defining run_pipeline(X_train, y_train, X_test, confusion, observed) in one ```python code block.\
"""
...
def score(source, data, ref, params):
    ...
    (X_train, X_test, y_train, y_test), (ideal, confusion, scales, observed) = data
    ...
    if bypass:
        ...
        result, error = _run_model_function(source, "run_pipeline", (X_train, y_train, X_test, confusion, observed), timeout=600)
    acc, l1, mit_ok = 0.0, float("inf"), False
    if error is None:
        try:
            acc = float(accuracy_score(y_test, np.asarray(result["predictions"]).ravel()))
            m = np.asarray(result["mitigated"], dtype=float).ravel()
            mit_ok = m.shape == ideal.shape and abs(m.sum() - 1) < 1e-6 and np.all(m >= -1e-9); l1 = float(np.abs(m - ideal).sum())
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(not bypass, "Must not hand .fidelity_quantum_kernel to QSVC: it bypasses whitening, normalize and bandwidth"),
              (error is None, f"Repaired code must run. Got: {error}"),
              (acc >= ref["acc_bar"], f"Accuracy >= {ref['acc_bar']} (correct pipeline {ref['correct_acc']:.2f}). Got {acc:.3f}"),
              (mit_ok and l1 <= ref["l1_bar"], f"mitigated must be a valid distribution from readout-correction + ZNE over all scales, L1 <= {ref['l1_bar']} (correct {ref['correct_l1']:.4f}). Got {l1:.4f}")]
    return {"accuracy": acc, "l1": l1, "error": error}, checks



@kbench.task(name="qmlfb-pipeline-repair-3bugs-b", description='Find and fix the planted bugs in a quantum kernel + mitigation pipeline.')
def qmlfb_pipeline_repair_3bugs_b(llm) -> float:
    _ensure_qmlf()
    data = (make_dataset("blobs", n_samples=80, n_features=5, seed=32, class_sep=1.4), make_mitigation(seed=3, n_states=4, base_p=0.12, readout_err=0.08, scales=(1, 2, 3)))
    ...
    # Kaggle builds the leaderboard from the RETURNED value and only accepts a
    # number or a bool -- a dict is recorded as resultCase "none", which is why
    # every task scored 0.00 while its assertions were passing. Returning the
    # fraction of checks passed gives partial credit, so a model that gets 2 of
    # 3 is distinguishable from one that gets 0. The metrics dict stays in
    # `metrics` for the run log; it just is not what Kaggle scores.
    return _fraction_passed(checks)


# %%
qmlfb_pipeline_repair_3bugs_b.run(kbench.llm)
