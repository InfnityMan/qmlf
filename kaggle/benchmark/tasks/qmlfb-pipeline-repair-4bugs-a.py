...
Planted: n_qubits disagrees with the 5-feature data; raw fidelity kernel handed to QSVC (discards whitening/normalize/bandwidth); mis-cased strategy name; 1-D input to ZNE. Correct pipeline: acc 0.95, L1 0.0322.
"""
...
# %%
REF = {'correct_acc': 0.95, 'correct_l1': 0.03223016325855116, 'acc_bar': 0.88, 'l1_bar': 0.0583, 'keep': True}
PARAMS = {'bugs': ['width', 'bypass', 'casing', 'zne1d']}
...
@kbench.task(name="qmlfb-pipeline-repair-4bugs-a", description='Find and fix the planted bugs in a quantum kernel + mitigation pipeline.')
def qmlfb_pipeline_repair_4bugs_a(llm) -> float:
    _ensure_qmlf()
    data = (make_dataset("blobs", n_samples=80, n_features=5, seed=29, class_sep=1.4), make_mitigation(seed=0, n_states=4, base_p=0.12, readout_err=0.08, scales=(1, 2, 3)))
...
# %%
qmlfb_pipeline_repair_4bugs_a.run(kbench.llm)
