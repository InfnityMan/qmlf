...
Planted: n_qubits disagrees with the 5-feature data; raw fidelity kernel handed to QSVC (discards whitening/normalize/bandwidth); readout mitigator used without .fit(confusion); 1-D input to ZNE. Correct pipeline: acc 0.85, L1 0.0138.
"""
...
@kbench.task(name="qmlfb-pipeline-repair-4bugs-b", description='Find and fix the planted bugs in a quantum kernel + mitigation pipeline.')
def qmlfb_pipeline_repair_4bugs_b(llm) -> float:
    _ensure_qmlf()
    data = (make_dataset("blobs", n_samples=80, n_features=5, seed=33, class_sep=1.4), make_mitigation(seed=4, n_states=4, base_p=0.12, readout_err=0.08, scales=(1, 2, 3)))
...
# %%
qmlfb_pipeline_repair_4bugs_b.run(kbench.llm)
