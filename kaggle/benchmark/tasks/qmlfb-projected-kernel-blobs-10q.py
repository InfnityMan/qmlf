...
10 qubits. Naive acc 0.50 (offdiag 0.0011); projected kernel 0.80 (offdiag 0.38).
"""
...
# %%
REF = {'naive_acc': 0.5, 'naive_offdiag': 0.0010664380108999123, 'ceiling_acc': 0.8, 'ceiling_offdiag': 0.3763214556076289, 'acc_bar': 0.72, 'keep': True}
...
Scoring: accuracy >= 0.72 AND clf.diagnose()["offdiag_mean"] >= 0.10. At this qubit count a global fidelity kernel with default settings collapses toward the identity (Thanasilp et al. 2024); think about which kernel family survives. Return only one ```python code block.\
"""
...
@kbench.task(name="qmlfb-projected-kernel-blobs-10q", description='Escape exponential concentration at this qubit count without reducing dimensionality.')
def qmlfb_projected_kernel_blobs_10q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=120, n_features=10, seed=12, class_sep=1.5)
...
# %%
qmlfb_projected_kernel_blobs_10q.run(kbench.llm)
