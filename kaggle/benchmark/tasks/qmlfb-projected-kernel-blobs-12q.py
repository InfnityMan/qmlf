# %%
"""Tier A -- Projected kernel at scale (no dimensionality reduction)

12 qubits. Naive acc 0.43 (offdiag 0.0003); projected kernel 0.67 (offdiag 0.37).
"""
...
# %%
REF = {'naive_acc': 0.43333333333333335, 'naive_offdiag': 0.0002876136156024597, 'ceiling_acc': 0.6666666666666666, 'ceiling_offdiag': 0.37110878412291776, 'acc_bar': 0.59, 'keep': True}
PARAMS = {'kind': 'blobs', 'n_features': 12}



PROMPT = """\
Binary classification with 12 continuous features. Encode ALL features on 12 qubits -- do not reduce dimensionality (qmlf.QuantumClassifier(..., max_qubits=None)). `qmlf` is installed; QuantumClassifier parameters: kernel ('fidelity'|'projected'), mode ('ZZ'|'mahalanobis'|'fisher' -- the preprocessing, default 'ZZ'), feature_map ('zz'|'z'), bandwidth ('auto'|'median'|a number), max_qubits; methods fit/predict/diagnose(). Write
    def solve(X_train, y_train, X_test):
        return {"predictions": <test predictions>, "classifier": <the fitted qmlf.QuantumClassifier>}
Scoring: accuracy >= 0.59 AND clf.diagnose()["offdiag_mean"] >= 0.10. At this qubit count a global fidelity kernel with default settings collapses toward the identity (Thanasilp et al. 2024); think about which kernel family survives. Return only one ```python code block.\
"""
...
def score(source, data, ref, params):
    ...
    if error is None:
        try:
            preds = np.asarray(result["predictions"]).ravel(); clf = result["classifier"]
            ok_type = isinstance(clf, qmlf.QuantumClassifier)
            acc = float(accuracy_score(y_test, preds)); offdiag = float(clf.diagnose()["offdiag_mean"]); n_qubits = int(clf.n_qubits_)
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    nf = params["n_features"]
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (ok_type, "Must return a fitted qmlf.QuantumClassifier"),
              (n_qubits == nf, f"All {nf} features must be encoded (n_qubits_ == {nf}, no reduction). Got {n_qubits}"),
              (offdiag >= 0.10, f"Kernel must not be concentrated: offdiag >= 0.10 (naive {ref['naive_offdiag']:.4f}). Got {offdiag:.4f}"),
              (acc >= ref["acc_bar"], f"Accuracy >= {ref['acc_bar']} (naive {ref['naive_acc']:.2f}, tuned {ref['ceiling_acc']:.2f}). Got {acc:.3f}")]
    return {"accuracy": acc, "offdiag_mean": offdiag, "n_qubits": n_qubits, "error": error}, checks



@kbench.task(name="qmlfb-projected-kernel-blobs-12q", description='Escape exponential concentration at this qubit count without reducing dimensionality.')
def qmlfb_projected_kernel_blobs_12q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=120, n_features=12, seed=14, class_sep=1.5)
...
# %%
qmlfb_projected_kernel_blobs_12q.run(kbench.llm)
