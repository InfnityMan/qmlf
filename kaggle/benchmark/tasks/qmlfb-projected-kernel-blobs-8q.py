...
8 qubits. Naive acc 0.40 (offdiag 0.0039); projected kernel 0.97 (offdiag 0.38).
"""
...
# %%
REF = {'naive_acc': 0.4, 'naive_offdiag': 0.0038585521340492902, 'ceiling_acc': 0.9666666666666667, 'ceiling_offdiag': 0.3798184324841955, 'acc_bar': 0.89, 'keep': True}
PARAMS = {'kind': 'blobs', 'n_features': 8}



PROMPT = """\
Binary classification with 8 continuous features. Encode ALL features on 8 qubits -- do not reduce dimensionality (qmlf.QuantumClassifier(..., max_qubits=None)). `qmlf` is installed; QuantumClassifier parameters: kernel ('fidelity'|'projected'), mode ('ZZ'|'mahalanobis'|'fisher' -- the preprocessing, default 'ZZ'), feature_map ('zz'|'z'), bandwidth ('auto'|'median'|a number), max_qubits; methods fit/predict/diagnose(). Write
...
@kbench.task(name="qmlfb-projected-kernel-blobs-8q", description='Escape exponential concentration at this qubit count without reducing dimensionality.')
def qmlfb_projected_kernel_blobs_8q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("blobs", n_samples=120, n_features=8, seed=11, class_sep=1.5)
...
# %%
qmlfb_projected_kernel_blobs_8q.run(kbench.llm)
