...
10 qubits. Naive acc 0.47 (offdiag 0.0019); projected kernel 0.83 (offdiag 0.38).
"""
...
# %%
REF = {'naive_acc': 0.4666666666666667, 'naive_offdiag': 0.0018931780926193784, 'ceiling_acc': 0.8333333333333334, 'ceiling_offdiag': 0.3757854626567479, 'acc_bar': 0.75, 'keep': True}
PARAMS = {'kind': 'moons', 'n_features': 10}



PROMPT = """\
Binary classification with 10 continuous features. Encode ALL features on 10 qubits -- do not reduce dimensionality (qmlf.QuantumClassifier(..., max_qubits=None)). `qmlf` is installed; QuantumClassifier parameters: kernel ('fidelity'|'projected'), mode ('ZZ'|'mahalanobis'|'fisher' -- the preprocessing, default 'ZZ'), feature_map ('zz'|'z'), bandwidth ('auto'|'median'|a number), max_qubits; methods fit/predict/diagnose(). Write
    def solve(X_train, y_train, X_test):
        return {"predictions": <test predictions>, "classifier": <the fitted qmlf.QuantumClassifier>}
Scoring: accuracy >= 0.75 AND clf.diagnose()["offdiag_mean"] >= 0.10. At this qubit count a global fidelity kernel with default settings collapses toward the identity (Thanasilp et al. 2024); think about which kernel family survives. Return only one ```python code block.\
"""
...
@kbench.task(name="qmlfb-projected-kernel-moons-10q", description='Escape exponential concentration at this qubit count without reducing dimensionality.')
def qmlfb_projected_kernel_moons_10q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("moons", n_samples=120, n_features=10, seed=16, class_sep=1.5)
...
# %%
qmlfb_projected_kernel_moons_10q.run(kbench.llm)
