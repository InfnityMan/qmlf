...
10 qubits. Naive acc 0.60 (offdiag 0.0012); projected kernel 0.97 (offdiag 0.42).
"""
...
# %%
REF = {'naive_acc': 0.6, 'naive_offdiag': 0.0011975887559825546, 'ceiling_acc': 0.9666666666666667, 'ceiling_offdiag': 0.42189192536863745, 'acc_bar': 0.89, 'keep': True}
PARAMS = {'kind': 'shift', 'n_features': 10}
...
Scoring: accuracy >= 0.89 AND clf.diagnose()["offdiag_mean"] >= 0.10. At this qubit count a global fidelity kernel with default settings collapses toward the identity (Thanasilp et al. 2024); think about which kernel family survives. Return only one ```python code block.\
"""
...
@kbench.task(name="qmlfb-projected-kernel-shift-10q", description='Escape exponential concentration at this qubit count without reducing dimensionality.')
def qmlfb_projected_kernel_shift_10q(llm) -> float:
    _ensure_qmlf()
    data = make_dataset("shift", n_samples=120, n_features=10, seed=13, class_sep=1.5)
...
# %%
qmlfb_projected_kernel_shift_10q.run(kbench.llm)
