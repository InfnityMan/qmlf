...
Raw 0.129, readout-only 0.046, full pipeline 0.0000 (L1 to ideal).
"""
...
@kbench.task(name="qmlfb-readout-zne-2states-seed5", description='Chain readout correction and ZNE correctly to recover an ideal distribution.')
def qmlfb_readout_zne_2states_seed5(llm) -> float:
    _ensure_qmlf()
    data = make_mitigation(seed=5, n_states=2, base_p=0.1, readout_err=0.1, scales=(1, 3, 5))
...
# %%
qmlfb_readout_zne_2states_seed5.run(kbench.llm)
