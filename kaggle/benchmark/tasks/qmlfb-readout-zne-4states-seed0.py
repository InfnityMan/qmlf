...
Raw 0.076, readout-only 0.050, full pipeline 0.0322 (L1 to ideal).
"""
...
Scoring: L1 distance to the true ideal <= 0.0352 (readout-only reaches ~0.050; raw ~0.076). Return only one ```python code block.\
"""
...
@kbench.task(name="qmlfb-readout-zne-4states-seed0", description='Chain readout correction and ZNE correctly to recover an ideal distribution.')
def qmlfb_readout_zne_4states_seed0(llm) -> float:
    _ensure_qmlf()
    data = make_mitigation(seed=0, n_states=4, base_p=0.12, readout_err=0.08, scales=(1, 2, 3))
...
# %%
qmlfb_readout_zne_4states_seed0.run(kbench.llm)
