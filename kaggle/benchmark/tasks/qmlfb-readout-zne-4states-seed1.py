...
Raw 0.207, readout-only 0.096, full pipeline 0.0532 (L1 to ideal).
"""
...
# %%
REF = {'raw': 0.20712022183170134, 'readout_only': 0.0961545198883066, 'pipeline': 0.05324540186361362, 'l1_bar': 0.0673, 'keep': True}
...
Scoring: L1 distance to the true ideal <= 0.0673 (readout-only reaches ~0.096; raw ~0.207). Return only one ```python code block.\
"""
...
@kbench.task(name="qmlfb-readout-zne-4states-seed1", description='Chain readout correction and ZNE correctly to recover an ideal distribution.')
def qmlfb_readout_zne_4states_seed1(llm) -> float:
    _ensure_qmlf()
    data = make_mitigation(seed=1, n_states=4, base_p=0.08, readout_err=0.12, scales=(1, 2, 3))
...
# %%
qmlfb_readout_zne_4states_seed1.run(kbench.llm)
