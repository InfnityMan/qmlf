...
Raw 0.181, readout-only 0.120, full pipeline 0.0324 (L1 to ideal).
"""
...
Scoring: L1 distance to the true ideal <= 0.0634 (readout-only reaches ~0.120; raw ~0.181). Return only one ```python code block.\
"""
...
@kbench.task(name="qmlfb-readout-zne-4states-seed3", description='Chain readout correction and ZNE correctly to recover an ideal distribution.')
def qmlfb_readout_zne_4states_seed3(llm) -> float:
    _ensure_qmlf()
    data = make_mitigation(seed=3, n_states=4, base_p=0.15, readout_err=0.05, scales=(1, 2, 4))
...
# %%
qmlfb_readout_zne_4states_seed3.run(kbench.llm)
