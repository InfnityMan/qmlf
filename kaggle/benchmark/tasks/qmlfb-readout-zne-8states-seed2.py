# %%
"""Tier D -- Readout correction + zero-noise extrapolation

Raw 0.101, readout-only 0.064, full pipeline 0.0193 (L1 to ideal).
"""
...
# %%
REF = {'raw': 0.10102942038704962, 'readout_only': 0.06429008125497024, 'pipeline': 0.019268962299304807, 'l1_bar': 0.0397, 'keep': True}
PARAMS = {'n_states': 8, 'scales': [1, 1.5, 2]}



PROMPT = """\
Measurements of a 8-outcome circuit at noise-scale factors [1, 1.5, 2] (gate noise amplified by those factors), plus a readout calibration confusion matrix A (row k = observed distribution when basis state k was prepared). Recover the ideal distribution. qmlf.create_advanced_noise_mitigator(strategy=...) with strategy in {'readout','depolarizing','zne'} (lower-case): 'readout' needs .fit(A) then .mitigate(distribution); 'zne' needs a 2-D (n_scales, n_states) stack of distributions at increasing noise and you may set its .scale_factors. Apply the stages in the physically correct order. Write
    def mitigate(confusion, scales, observed):   # observed rows = scales
        return <length-8 probability estimate of the ideal distribution>
Scoring: L1 distance to the true ideal <= 0.0397 (readout-only reaches ~0.064; raw ~0.101). Return only one ```python code block.\
"""


def score(source, data, ref, params):
    import numpy as np
    ideal, confusion, scales, observed = data
    ...
    if error is None:
        try:
            ...
            valid = est.shape == ideal.shape and np.all(est >= -1e-9) and abs(est.sum() - 1) < 1e-6
            l1 = float(np.abs(est - ideal).sum())
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (valid, f"Result must be a valid length-{len(ideal)} probability distribution"),
              (l1 <= ref["l1_bar"], f"L1 to ideal must be <= {ref['l1_bar']} (raw {ref['raw']:.3f}, readout-only {ref['readout_only']:.3f}, full pipeline {ref['pipeline']:.4f}). Got {l1:.4f}")]
    return {"l1_to_ideal": l1, "error": error}, checks



@kbench.task(name="qmlfb-readout-zne-8states-seed2", description='Chain readout correction and ZNE correctly to recover an ideal distribution.')
def qmlfb_readout_zne_8states_seed2(llm) -> float:
    _ensure_qmlf()
    data = make_mitigation(seed=2, n_states=8, base_p=0.1, readout_err=0.06, scales=(1, 1.5, 2))
...
# %%
qmlfb_readout_zne_8states_seed2.run(kbench.llm)
