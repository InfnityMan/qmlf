# %%
"""Tier F -- Quantum kernel regression of a physical response

quadratic: naive R2 -0.55, fidelity auto 0.98, projected 0.87.
"""
...
# %%
REF = {'naive_r2': -0.548219887858842, 'fid_r2': 0.9765675342929727, 'proj_r2': 0.8655141935450628, 'r2_bar': 0.88, 'keep': True}
PARAMS = {'kind': 'quadratic', 'label': 'quadratic-interaction', 'n_features': 6}
...
class Plan(BaseModel):
    ...
    rationale: str = Field(description="Why this kernel family generalises on this target")


PROMPT = """\
Regression of a smooth quadratic-interaction response y from 6 continuous inputs. Configure qmlf.QuantumRegressor (kernel ridge on a quantum kernel): kernel ('fidelity'|'projected'|'auto'), mode, bandwidth ('auto' CV over MSE, 'median', or a number), max_qubits, alpha. Scoring: held-out R^2 >= 0.88. The global fidelity kernel and the projected kernel family (local reduced density matrices, Huang et al. 2021) generalise differently on oscillatory targets. Choose the configuration.\
"""
...
def score(plan, data, ref, params):
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            reg = qmlf.QuantumRegressor(kernel=plan.kernel, mode=plan.mode, bandwidth=bandwidth, max_qubits=int(plan.max_qubits), alpha=float(plan.alpha)).fit(X_train, y_train)
        r2 = float(r2_score(y_test, reg.predict(X_test)))
    except Exception as exc:
        error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Configuration must be valid. Got: {error}"),
              (r2 >= ref["r2_bar"], f"R^2 >= {ref['r2_bar']} (naive {ref['naive_r2']:.2f}, fidelity auto {ref['fid_r2']:.2f}, projected {ref['proj_r2']:.2f}). Got {r2:.3f}")]
    return {"r2": r2, "plan": plan.model_dump(), "error": error}, checks



@kbench.task(name="qmlfb-regression-quadratic", description='Regress a physical response with a quantum kernel ridge that generalises.')
def qmlfb_regression_quadratic(llm) -> float:
    _ensure_qmlf()
    data = make_regression("quadratic", n_samples=90, n_features=6, seed=93)
...
# %%
qmlfb_regression_quadratic.run(kbench.llm)
