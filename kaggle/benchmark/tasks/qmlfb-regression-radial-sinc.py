...
sinc: naive R2 -0.19, fidelity auto 0.92, projected 0.82.
"""
...
# %%
REF = {'naive_r2': -0.1869038660489175, 'fid_r2': 0.9185187484186191, 'proj_r2': 0.8203669719224876, 'r2_bar': 0.82, 'keep': True}
PARAMS = {'kind': 'sinc', 'label': 'radial sinc', 'n_features': 6}
...
PROMPT = """\
Regression of a smooth radial sinc response y from 6 continuous inputs. Configure qmlf.QuantumRegressor (kernel ridge on a quantum kernel): kernel ('fidelity'|'projected'|'auto'), mode, bandwidth ('auto' CV over MSE, 'median', or a number), max_qubits, alpha. Scoring: held-out R^2 >= 0.82. The global fidelity kernel and the projected kernel family (local reduced density matrices, Huang et al. 2021) generalise differently on oscillatory targets. Choose the configuration.\
"""
...
@kbench.task(name="qmlfb-regression-radial-sinc", description='Regress a physical response with a quantum kernel ridge that generalises.')
def qmlfb_regression_radial_sinc(llm) -> float:
    _ensure_qmlf()
    data = make_regression("sinc", n_samples=90, n_features=6, seed=91)
...
# %%
qmlfb_regression_radial_sinc.run(kbench.llm)
