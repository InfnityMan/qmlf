...
Real transpiler depth 31, 2q gates 24; analytic estimate claims depth 16.
"""
...
# %%
REF = {'t_depth': 31, 't_2q': 24, 'e_depth': 16, 'keep': True}
PARAMS = {'n': 4, 'reps': 2, 'ent': 'full', 'strength': 0.65}



PROMPT = """\
A hardware team asks for the post-optimisation resources of a 4-qubit ZZ feature map (reps=2, entanglement='full'). qmlf.create_advanced_nisq_optimizer(optimization_strength=0.65) has optimize_transpile(circuit) -> REAL qiskit transpilation (dict: 'mode'='transpiled', 'optimized_depth', 'optimized_two_qubit_gates', ...) and optimize_circuit(depth, gate_count, two_qubit_gates) -> analytic PROJECTION ('mode'='estimate'). Write
    def report(circuit):
...
@kbench.task(name="qmlfb-transpile-4q-2reps-full", description='Report real transpiled resources, not the analytic projection, and label each correctly.')
def qmlfb_transpile_4q_2reps_full(llm) -> float:
    _ensure_qmlf()
    data = __import__("qiskit.circuit.library", fromlist=["zz_feature_map"]).zz_feature_map(4, reps=2, entanglement="full")
...
# %%
qmlfb_transpile_4q_2reps_full.run(kbench.llm)
