...
Real transpiler depth 40, 2q gates 40; analytic estimate claims depth 16.
"""
...
# %%
REF = {'t_depth': 40, 't_2q': 40, 'e_depth': 16, 'keep': True}
PARAMS = {'n': 5, 'reps': 2, 'ent': 'full', 'strength': 1.0}



PROMPT = """\
A hardware team asks for the post-optimisation resources of a 5-qubit ZZ feature map (reps=2, entanglement='full'). qmlf.create_advanced_nisq_optimizer(optimization_strength=1.0) has optimize_transpile(circuit) -> REAL qiskit transpilation (dict: 'mode'='transpiled', 'optimized_depth', 'optimized_two_qubit_gates', ...) and optimize_circuit(depth, gate_count, two_qubit_gates) -> analytic PROJECTION ('mode'='estimate'). Write
    def report(circuit):
...
@kbench.task(name="qmlfb-transpile-5q-2reps-full", description='Report real transpiled resources, not the analytic projection, and label each correctly.')
def qmlfb_transpile_5q_2reps_full(llm) -> float:
    _ensure_qmlf()
    data = __import__("qiskit.circuit.library", fromlist=["zz_feature_map"]).zz_feature_map(5, reps=2, entanglement="full")
...
# %%
qmlfb_transpile_5q_2reps_full.run(kbench.llm)
