# %%
"""Tier C -- Transpiled reality vs analytic estimate

Real transpiler depth 51, 2q gates 30; analytic estimate claims depth 16.
"""
...
# %%
REF = {'t_depth': 51, 't_2q': 30, 'e_depth': 16, 'keep': True}
PARAMS = {'n': 5, 'reps': 3, 'ent': 'circular', 'strength': 0.65}



PROMPT = """\
A hardware team asks for the post-optimisation resources of a 5-qubit ZZ feature map (reps=3, entanglement='circular'). qmlf.create_advanced_nisq_optimizer(optimization_strength=0.65) has optimize_transpile(circuit) -> REAL qiskit transpilation (dict: 'mode'='transpiled', 'optimized_depth', 'optimized_two_qubit_gates', ...) and optimize_circuit(depth, gate_count, two_qubit_gates) -> analytic PROJECTION ('mode'='estimate'). Write
    def report(circuit):
        return {"measured_depth": <int from the real transpiler>, "measured_two_qubit_gates": <int from the real transpiler>, "estimated_depth": <int from the analytic estimate>, "mode_used_for_measured": <'mode' string of the result you call measured>}
The two paths disagree for this circuit. Report reality as reality. Return only one ```python code block.\
"""
...
def score(source, data, ref, params):
    ...
    if error is None:
        try:
            md = int(result["measured_depth"]); mg = int(result["measured_two_qubit_gates"]); ed = int(result["estimated_depth"]); mode = result["mode_used_for_measured"]
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    checks = [(error is None, f"Model code must run. Got: {error}"),
              (mode == "transpiled", f"Measured numbers must come from the real transpiler (mode 'transpiled'). Got {mode!r}"),
              (md == ref["t_depth"] and mg == ref["t_2q"], f"Measured depth/2q-gates must be {ref['t_depth']}/{ref['t_2q']}. Got {md}/{mg}"),
              (ed == ref["e_depth"], f"Estimated depth (analytic path) must be {ref['e_depth']}. Got {ed}")]
    return {"measured_depth": md, "measured_two_qubit_gates": mg, "estimated_depth": ed, "mode": mode, "error": error}, checks



@kbench.task(name="qmlfb-transpile-5q-3reps-circular", description='Report real transpiled resources, not the analytic projection, and label each correctly.')
def qmlfb_transpile_5q_3reps_circular(llm) -> float:
    _ensure_qmlf()
    data = __import__("qiskit.circuit.library", fromlist=["zz_feature_map"]).zz_feature_map(5, reps=3, entanglement="circular")
...
# %%
qmlfb_transpile_5q_3reps_circular.run(kbench.llm)
