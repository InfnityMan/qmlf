...
Real transpiler depth 20, 2q gates 12; analytic estimate claims depth 13.
"""
...
# %%
REF = {'t_depth': 20, 't_2q': 12, 'e_depth': 13, 'keep': True}
PARAMS = {'n': 6, 'reps': 1, 'ent': 'circular', 'strength': 0.65}



PROMPT = """\
A hardware team asks for the post-optimisation resources of a 6-qubit ZZ feature map (reps=1, entanglement='circular'). qmlf.create_advanced_nisq_optimizer(optimization_strength=0.65) has optimize_transpile(circuit) -> REAL qiskit transpilation (dict: 'mode'='transpiled', 'optimized_depth', 'optimized_two_qubit_gates', ...) and optimize_circuit(depth, gate_count, two_qubit_gates) -> analytic PROJECTION ('mode'='estimate'). Write
    def report(circuit):
...
@kbench.task(name="qmlfb-transpile-6q-1rep-circular", description='Report real transpiled resources, not the analytic projection, and label each correctly.')
def qmlfb_transpile_6q_1rep_circular(llm) -> float:
    _ensure_qmlf()
    data = __import__("qiskit.circuit.library", fromlist=["zz_feature_map"]).zz_feature_map(6, reps=1, entanglement="circular")
...
# %%
qmlfb_transpile_6q_1rep_circular.run(kbench.llm)
