...
exact -6.491856; operator ground state -27.630314; VQE from zeros -27.619598.
"""
...
# %%
REF = {'exact': -6.491856285328178, 'operator_min': -27.63031399521005, 'vqe_zeros': -27.6195982626086, 'keep': True}
PARAMS = {'n_atoms': 3}



PROMPT = """\
Two ground-state energies for a 3-atom geometry with charges: the exact diagonalisation of qmlf's model Hamiltonian and a genuine VQE estimate on the corresponding spin operator. qmlf.create_advanced_chem_layer(num_atoms=3) exposes compute_ground_state_energy(coordinates, charges) and compute_vqe_energy(coordinates, charges, initial_point=None) (real_amplitudes reps=2 + COBYLA; a RANDOM start by default, so pass a fixed initial_point vector of length = ansatz parameter count for determinism). Write
...
@kbench.task(name="qmlfb-vqe-3atoms", description='Deliver exact and deterministic VQE energies that respect the variational bound.')
def qmlfb_vqe_3atoms(llm) -> float:
    _ensure_qmlf()
    data = make_geometry(seed=1, n_atoms=3)
...
# %%
qmlfb_vqe_3atoms.run(kbench.llm)
